...
import { debitCreditAPI, vendorAPI } from '../services/api';
...
const VENDOR_DEBIT_REASONS = [
  ...
  { label: 'Return (Goods Returned to Vendor)', value: 'Return' },
...
const VENDOR_CREDIT_REASONS = [
  ...
  { label: 'Late Payment Interest', value: 'Late Payment Interest' },
  { label: 'Service Charge', value: 'Service Charge' },
  { label: 'Missed Charges', value: 'Missed Charges' },
  { label: 'Other', value: 'Other' },
];

const VendorDebitCreditScreen = ({ route, navigation }) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [vendorData, balanceData, debitData, creditData] = await Promise.all([
        vendorAPI.getById(vendorId),
        debitCreditAPI.getVendorBalance(vendorId),
        debitCreditAPI.getVendorDebitNotes(vendorId),
        debitCreditAPI.getVendorCreditNotes(vendorId),
      ]);

      const poData = await debitCreditAPI.getVendorPurchaseOrders(vendorId).catch(() => []);
  ...
  const handleCreateDebit = async () => {
    ...
    try {
      await debitCreditAPI.createVendorDebitNote({
        ...
        po_id: debitFormData.po_id ? Number(debitFormData.po_id) : null,
        created_by: 'mobile',
      });
  ...
  const handleCreateCredit = async () => {
    ...
    try {
      await debitCreditAPI.createVendorCreditNote({
        ...
        po_id: creditFormData.po_id ? Number(creditFormData.po_id) : null,
        created_by: 'mobile',
      });
          ...
          <View style={styles.balanceContainer}>
            <View style={styles.balanceCard}>
              ...
              <Text style={styles.balanceSubtext}>
                {balance.balance > 0 ? 'Amount Payable' : 'Advance Given'}
              </Text>
            </View>
              ...
              {showDebitForm && (
                <View style={styles.formBody}>
                  ...
                  <PlatformPicker
                    ...
                    items={VENDOR_DEBIT_REASONS}
                    placeholder={null}
                    wrapperStyle={styles.pickerWrapper}
                    pickerStyle={styles.picker}
                    modalTitle="Reason"
                  />
                  <Text style={styles.inputLabel}>Purchase Order (Optional)</Text>
                  <PlatformPicker
                    selectedValue={debitFormData.po_id}
                    onValueChange={(value) =>
                      setDebitFormData((prev) => ({ ...prev, po_id: value }))
                    }
                    items={[
                      { label: '-- Select PO --', value: '' },
                      ...(purchaseOrders || []).map((po) => ({
                        label: `#${po.po_number || po.id} • ₹${formatCurrency(po.total_amount)}`,
                        value: po.id,
                      })),
                    ]}
                    placeholder={null}
              ...
              debitNotes.map((note) => (
                <View key={note.id} style={styles.card}>
                  <View style={styles.cardHeader}>
                    <Text style={styles.cardTitle}>VDN-{note.id}</Text>
                    <View
                      style={[
                        styles.statusBadge,
                  ...
                  <View style={styles.cardBody}>
                    ...
                    {note.po_id && (
                      <Text style={styles.cardMeta}>PO: #{note.po_id}</Text>
                    )}
                    {note.description && (
          ...
          <View>
            <View style={styles.formCard}>
              ...
              {showCreditForm && (
                <View style={styles.formBody}>
                  ...
                  <Text style={styles.inputLabel}>Purchase Order (Optional)</Text>
                  <PlatformPicker
                    selectedValue={creditFormData.po_id}
                    onValueChange={(value) =>
                      setCreditFormData((prev) => ({ ...prev, po_id: value }))
                    }
                    items={[
                      { label: '-- Select PO --', value: '' },
                      ...(purchaseOrders || []).map((po) => ({
            ...
            {creditNotes.length === 0 ? (
              ...
              creditNotes.map((note) => (
                <View key={note.id} style={styles.card}>
                  <View style={styles.cardHeader}>
                    <Text style={styles.cardTitle}>VCN-{note.id}</Text>
                    <View
                      style={[
                        styles.statusBadge,
                        { backgroundColor: getStatusColor(note.status) },
...
export default VendorDebitCreditScreen;
